...
class BaseGameMode(ABC):
    ...
    def __init__(self, checkpoints, total_laps=3, start_grid=None):
        ...
        self.player_states: dict[str, _PlayerRaceState] = {} # player_id -> _PlayerRaceState
        self.leaderboard: list[_PlayerRaceState] = []  # ordered
    ...
    def on_enter(self, players, local_player_id="local"):
        ...
        if local_player_id not in self.player_states:
            self.player_states[local_player_id] = _PlayerRaceState(local_player_id, name="You")
...
class SimpleRace(BaseGameMode):
    ...
    def update(self, dt, players, my_car, is_host): # per frame upadte
