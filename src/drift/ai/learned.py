...
def _observation_from_car(car, track_mask) -> np.ndarray:
    ...
    rays = []
    for deg in [-80, -40, -20, -10, -5, 0, 5, 10, 20, 40, 80]:
        _, _, dist, hit = raycast_black_mask(
            track_mask, car.x * 0.5, car.y * 0.5,
            car.angle + math.radians(deg), 400.0,
        )
        rays.append(dist / 400.0 if hit else 1.0)
    return np.array(state + rays, dtype=np.float32)


def apply_learned_controls(car, base_controls: Dict[str, float], map_num: int, difficulty: str) -> Dict[str, float]:
    policy = get_policy(map_num, difficulty)
    if policy is None:
        return base_controls
    action = policy.network.forward(_observation_from_car(car, policy.track_mask))
    learned = {
