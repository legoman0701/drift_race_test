# Drift Race Test

Top-down drift racing game with online multiplayer, AI opponents, and realistic physics.

**Website**: [https://nmax0.github.io/drift-site/](https://nmax0.github.io/drift-site/index.html)  
For a complete overview, features, and controls.

---

## Getting Started

### Quick Start
1. Download the latest release from [Downloads](https://nmax0.github.io/drift-site/download.html)
2. Extract and run `Drift.exe` (Windows)
3. For installation help, visit the [Help Page](https://nmax0.github.io/drift-site/help.html)

### Controls & Features
See the [project website](https://nmax0.github.io/drift-site/index.html) for:
- Complete controls reference
- Feature documentation
- Gameplay tips
- Multiplayer setup

---

## Changelog

Full changelog available at: [https://nmax0.github.io/drift-site/changelog.html](https://nmax0.github.io/drift-site/changelog.html)

---

## Sources

Audio assets:
- Turbo blow-off valve: [https://samplefocus.com/samples/blow-off-valve-supra-stutututu](https://samplefocus.com/samples/blow-off-valve-supra-stutututu)
